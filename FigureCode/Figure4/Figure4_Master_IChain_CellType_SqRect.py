...
for to_proofread in [True, False]:
    ...
    pt_root_id_to_classification = post_cell_table.set_index('pt_root_id')['cell_type'].to_dict()
    ...
    for _, row in tqdm(enumerate(two_chain_results_array)):
        ...
        # Get synapse weights for connections j -> k and k -> i
            ...
            w_ki = summed_size_connectome_df.loc[mid_cell, post_cell] * (9 * 9 * 45) / (10**9) # cubic micrometers
        # Determine chain type (excitatory if middle cell is in excitatory set, else inhibitory)
            if pt_root_id_to_classification[mid_cell] == 'PTC' or pt_root_id_to_classification[mid_cell] == 'ProxTC':
                count_PTC += 1
                for connection_type in comparison_functions:
            ...
            if pt_root_id_to_classification[mid_cell] == 'ITC' or pt_root_id_to_classification[mid_cell] == 'InhTC':
                count_ITC += 1
                for connection_type in comparison_functions:
                    if connection_type(pre_cell, post_cell, A):
                        ...
                        W_chain_by_type['INH'][key] = W_chain_by_type['INH'].get(key, 0) + (w_jk * w_ki)
                        B_chain_by_type['INH'][key] = 1
            if pt_root_id_to_classification[mid_cell] == 'STC' or pt_root_id_to_classification[mid_cell] == 'SparTC':
                count_STC += 1
                for connection_type in comparison_functions:
                    if connection_type(pre_cell, post_cell, A):
                        ...
                        W_chain_by_type['INH'][key] = W_chain_by_type['INH'].get(key, 0) + (w_jk * w_ki)
                        B_chain_by_type['INH'][key] = 1
            if pt_root_id_to_classification[mid_cell] == 'DTC' or pt_root_id_to_classification[mid_cell] == 'DistTC':
                count_DTC += 1
                for connection_type in comparison_functions:
                    if connection_type(pre_cell, post_cell, A):
                        ...
                        W_chain_by_type['INH'][key] = W_chain_by_type['INH'].get(key, 0) + (w_jk * w_ki)
                        B_chain_by_type['INH'][key] = 1
            elif pt_root_id_to_classification[mid_cell][0] == 'L' or pt_root_id_to_classification[mid_cell][1] == 'P':
                count_PYR += 1
                for connection_type in comparison_functions:
                    if connection_type(pre_cell, post_cell, A):
